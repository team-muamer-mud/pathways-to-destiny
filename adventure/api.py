...
def initialize(request):
    ...
    return JsonResponse({'uuid': uuid, 'world': worldRooms, 'name':player.user.username, "id" : room.id, "x": room.x, "y": room.y, 'title':room.title, 'description':room.description, 'players':players}, safe=True)
...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        players = room.playerNames(player_id)
        return JsonResponse({'name':player.user.username, 'title':room.title, "id" : room.id, "x": room.x, "y": room.y, 'description':room.description, 'players':players, 'error_msg':"You cannot move that way."}, safe=True)
