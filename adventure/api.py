...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        return JsonResponse({'name':player.user.username, 'title':nextRoom.title, "id" : nextRoom.id, "x": nextRoom.x, "y": nextRoom.y, 'n_to':nextRoom.n_to, 's_to': nextRoom.s_to, 'e_to':nextRoom.e_to, "w_to": nextRoom.w_to, 'description':nextRoom.description, 'players':players, 'error_msg':""}, safe=True)
    else:
        players = room.playerNames(player_id)
